# This file implements synthetic children providers and summaries for various VanGUI types for LLDB.
...
class VanVectorSynth(ArraySynthBase):
	def update(self):
...
class VanSpanSynth(ArraySynthBase):
	def update(self):
		data = self.valobj.GetChildMemberWithName("Data")
...
class VanGuiWindowSummary(object):
	def __init__(self, valobj, internal_dict):
		self.valobj = valobj
...
def __lldb_init_module(debugger, internal_dict):
	...
	add_synthetic("^VanVector<.+>$", VanVectorSynth)
	add_synthetic("^VanSpan<.+>$", VanSpanSynth)

	add_summary("^VanVec2$", "x=${var.x} y=${var.y}")
	add_summary("^VanVec4$", "x=${var.x} y=${var.y} z=${var.z} w=${var.w}")
	add_summary("^VanRect$", VanRectSummary)
	add_summary("^VanGuiWindow$", VanGuiWindowSummary)
