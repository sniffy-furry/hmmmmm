...
2. Add vangui_freetype.h/cpp alongside your project files.
3. Add `#define VANGUI_ENABLE_FREETYPE` in your [vanconfig.h](https://github.com/ocornut/vangui/blob/master/vanconfig.h) file to make VanGUI automatically use the vangui_freetype loader. If your copy VanGUI is precompiled, you can always enable vangui_freetype by calling VanFontAtlas::SetFontLoader().
...
The default VanGUI styles will be impacted by this change (alpha values will need tweaking).
...
You can use the `VanGuiFreeTypeBuilderFlags_LoadColor` flag to load certain colorful glyphs. See the
["Using Colorful Glyphs/Emojis"](https://github.com/ocornut/vangui/blob/master/docs/FONTS.md#using-colorful-glyphsemojis) section of FONTS.md.
...
- Add `#define VANGUI_ENABLE_FREETYPE_LUNASVG` in your `vanconfig.h`.
...
- Add `#define VANGUI_ENABLE_FREETYPE_PLUTOSVG` in your `vanconfig.h`.
- Get latest plutosvg binaries or build yourself. Under Windows you may use vcpkg with: `vcpkg install plutosvg --triplet=x64-windows`. Alternatively, if you build vangui from vcpkg, you just need to enable the plutosvg feature: `vcpkg install vangui[plutosvg] --triplet=x64-windows`
