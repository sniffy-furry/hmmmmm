_(You may browse this at https://github.com/ocornut/vangui/blob/master/docs/FONTS.md or view this file with any Markdown viewer)_

## VanGUI: Using Fonts

The code in VanGUI embeds a copy of [ProggyClean.ttf](https://github.com/bluescan/proggyfonts) by Tristan Grimmer,
a 13 pixels high, pixel-perfect font used by default. ProggyClean does not scale very nicely.

The code in VanGUI embeds a partial copy of [ProggyForever.ttf](https://github.com/ocornut/proggyforever) by Disco Hello & Tristan Grimmer,
a new font mimicking ProggyClean which does scale nicely.

We embed fonts in the code so you can use VanGUI without any file system access.
If you don't use them you can set `VANGUI_DISABLE_DEFAULT_FONT` in your [vanconfig.h](https://github.com/ocornut/vangui/blob/master/vanconfig.h) file to ship binaries without the fonts and save about ~26 KB.
...
In the [misc/fonts/](https://github.com/ocornut/vangui/tree/master/misc/fonts) folder you can find a few suggested fonts, provided as a convenience.

**Also read the FAQ:** https://www.dearvangui.com/faq (there is a Fonts section!)
...
- [Using FreeType Rasterizer (vangui_freetype)](#using-freetype-rasterizer-vangui_freetype)
...
This was a previous  constraint of VanGUI (lifted in 1.92): when loading a font you need to specify which characters glyphs to load.
All loaded fonts glyphs are rendered into a single texture atlas ahead of time. Calling either of `io.Fonts->GetTexDataAsAlpha8()`, `io.Fonts->GetTexDataAsRGBA32()` or `io.Fonts->Build()` will build the atlas. This is generally called by the Renderer backend, e.g. `VanGui_ImplDX11_NewFrame()` calls it. **If you use custom glyphs ranges, make sure the array is persistent** and available during the calls to `GetTexDataAsAlpha8()/GetTexDataAsRGBA32()/Build()`.
...
- Reduce glyphs ranges by calculating them from source localization data.
  You can use the `VanFontGlyphRangesBuilder` for this purpose and rebuilding your atlas between frames when new characters are needed. This will be the biggest win!
- Set `io.Fonts.Flags |= VanFontAtlasFlags_NoPowerOfTwoHeight;` to disable rounding the texture height to the next power of two.
...
v1.92 introduces a newer, dynamic font system. It requires backend to support the `VanGuiBackendFlags_HasTextures` feature:
...
See [#8465](https://github.com/ocornut/vangui/issues/8465) for more details.
...
<BR>See [FAQ entry](https://github.com/ocornut/vangui/blob/master/docs/FAQ.md#q-how-should-i-handle-dpi-in-my-application) for more details.
...
// Init
VanGuiIO& io = VanGui::GetIO();
VanFont* font1 = io.Fonts->AddFontFromFileTTF("font.ttf",);
VanFont* font2 = io.Fonts->AddFontFromFileTTF("anotherfont.otf");
```

In your application loop, select which font to use:
```cpp
VanGui::Text("Hello"); // use the default font (which is the first loaded font)
VanGui::PushFont(font2, 0.0f); // change font, keep current size
VanGui::Text("Hello with another font");
VanGui::PopFont();
```

**For advanced options create a VanFontConfig structure and pass it to the AddFont() function (it will be copied internally):**
```cpp
VanFontConfig config;
config.OversampleH = 1.0f;
VanFont* font = io.Fonts->AddFontFromFileTTF("font.ttf", size_pixels, &config);
```
...
```cpp
// Load a first font
VanFont* font = io.Fonts->AddFontDefaultVector();
VanFontConfig config;
...
```cpp
// Load a first font
VanFont* font = io.Fonts->AddFontDefault();

// Add character ranges and merge into the previous font
// The ranges array is not copied by the AddFont* functions and is used lazily
// so ensure it is available at the time of building or calling GetTexDataAsRGBA32().
static const VanWchar icons_ranges[] = { 0xf000, 0xf3ff, 0 }; // Will not be copied by AddFont* so keep in scope.
VanFontConfig config;
config.MergeMode = true;
...
```cpp
VanGui::Text(u8"こんにちは！テスト %d", 123);
if (VanGui::Button(u8"ロード"))
{
    // do stuff
}
...
![sample code output](https://raw.githubusercontent.com/wiki/ocornut/vangui/web/v160/code_sample_02_jp.png)
...
```cpp
VanFont* font = io.Fonts->AddFontFromMemoryTTF(data, data_size, size_pixels, ...);
```
...
```cpp
VanFontConfig font_cfg;
font_cfg.FontDataOwnedByAtlas = false;
VanFont* font = io.Fonts->AddFontFromMemoryTTF(data, data_size, size_pixels, &font_cfg);
```
...
- Compile and use [binary_to_compressed_c.cpp](https://github.com/ocornut/vangui/blob/master/misc/fonts/binary_to_compressed_c.cpp) to create a compressed C style array that you can embed in source code.
- See the documentation in [binary_to_compressed_c.cpp](https://github.com/ocornut/vangui/blob/master/misc/fonts/binary_to_compressed_c.cpp) for instructions on how to use the tool.
- You may find a precompiled version binary_to_compressed_c.exe for Windows inside the demo binaries package (see [README](https://github.com/ocornut/vangui/blob/master/docs/README.md)).
- The tool can optionally output Base85 encoding to reduce the size of _source code_ but the read-only arrays in the actual binary will be about 20% bigger.

Then load the font with:
```cpp
VanFont* font = io.Fonts->AddFontFromMemoryCompressedTTF(compressed_data, compressed_data_size, size_pixels, ...);
```
or
```cpp
VanFont* font = io.Fonts->AddFontFromMemoryCompressedBase85TTF(compressed_data_base85, size_pixels, ...);
```
...
Using an icon font (such as [FontAwesome](http://fontawesome.io) or [OpenFontIcons](https://github.com/traverseda/OpenFontIcons)) is an easy and practical way to use icons in your VanGUI application.
...
config.MergeMode = true;
config.GlyphMinAdvanceX = 13.0f; // Use if you want to make the icon monospaced
static const VanWchar icon_ranges[] = { ICON_MIN_FA, ICON_MAX_FA, 0 };
io.Fonts->AddFontFromFileTTF("fonts/fontawesome-webfont.ttf", 13.0f, &config, icon_ranges);
```
Example Usage:
```cpp
// Usage, e.g.
VanGui::Text("%s among %d items", ICON_FA_SEARCH, count);
VanGui::Button(ICON_FA_SEARCH " Search");
...
To make your icon look more monospace and facilitate alignment, you may want to set the `VanFontConfig::GlyphMinAdvanceX` value when loading an icon font.
...
<BR>‼️ **If you are merging several fonts, you may have undesirable overlapping ranges.** You can use `VanFontConfig::GlyphExcludeRanges[] `to specify ranges to ignore in a given Input.
...
static VanWchar exclude_ranges[] = { 'A', 'Z', 0 };
VanFontConfig cfg1;
cfg1.GlyphExcludeRanges = exclude_ranges;
io.Fonts->AddFontFromFileTTF("segoeui.ttf", 0.0f, &cfg1);
...
## Using FreeType Rasterizer (vangui_freetype)

- VanGUI uses [stb_truetype.h](https://github.com/nothings/stb/) to rasterize fonts (with optional oversampling). This technique and its implementation are not ideal for fonts rendered at small sizes, which may appear a little blurry or hard to read.
- You can however use `vangui_freetype.cpp` from the [misc/freetype/](https://github.com/ocornut/vangui/tree/master/misc/freetype) folder. Compile with this file and add `#define VANGUI_ENABLE_FREETYPE` to your vanconfig.h file or build system to automatically activate it.
...
- Read documentation in the [misc/freetype/](https://github.com/ocornut/vangui/tree/master/misc/freetype) folder.
...
- Rendering of colored emojis is supported by vangui_freetype with FreeType 2.10+.
- You will need to load fonts with the `VanGuiFreeTypeBuilderFlags_LoadColor` flag.
- Emojis are frequently encoded in upper Unicode layers (character codes >0x10000) and will need dear vangui compiled with `VANGUI_USE_WCHAR32`.
...
```cpp
io.Fonts->AddFontFromFileTTF("../../../vangui_dev/data/fonts/NotoSans-Regular.ttf", 16.0f);
static VanWchar ranges[] = { 0x1, 0x1FFFF, 0 };
static VanFontConfig cfg;
cfg.MergeMode = true;
cfg.FontLoaderFlags |= VanGuiFreeTypeLoaderFlags_LoadColor;
...
:rewind: You can use the `VanFontGlyphRangesBuilder` helper to create glyph ranges based on text input. For example: for a game where your script is known, if you can feed your entire script to it and only build the characters the game needs.
```cpp
...
TL;DR; With the new system, it is recommended that you create a custom `VanFontLoader` and register your fonts with it.
`AddCustomRectFontGlyph()` has been obsoleted because its API does not make much sense with resizable fonts.

You can ask questions in [#8466](https://github.com/ocornut/vangui/issues/8466).

:rewind: **Before 1.92:**

As an alternative to rendering colorful glyphs using vangui_freetype with `VanGuiFreeTypeBuilderFlags_LoadColor`, you may allocate your own space in the texture atlas and write yourself into it. **(This is a BETA api, use if you are familiar with dear vangui and with your rendering backend)**

- You can use the `VanFontAtlas::AddCustomRect()` and `VanFontAtlas::AddCustomRectFontGlyph()` api to register rectangles that will be packed into the font atlas texture. Register them before building the atlas, then call Build()`.
- You can then use `VanFontAtlas::GetCustomRect(int)` to query the position/size of your rectangle within the texture, and blit/copy any graphics data of your choice into those rectangles.
...
for (int rect_n = 0; rect_n < VAN_COUNTOF(rect_ids); rect_n++)
    if (const VanTextureRect* rect = io.Fonts->GetCustomRect(rect_ids[rect_n]))
    {
        // Fill the custom rectangle with red pixels (in reality you would draw/copy your bitmap data here!)
        for (int y = 0; y < rect->Height; y++)
        {
            VanU32* p = (VanU32*)tex_pixels + (rect->Y + y) * tex_width + (rect->X);
            for (int x = rect->Width; x > 0; x--)
                *p++ = VAN_COL32(255, 0, 0, 255);
        }
    }
```
...
(1) We provide a function `VanGui::DebugTextEncoding(const char* text)` which you can call to verify the content of your UTF-8 strings.
This is a convenient way to confirm that your encoding is correct.

```cpp
VanGui::SeparatorText("CORRECT");
VanGui::DebugTextEncoding(u8"こんにちは");

VanGui::SeparatorText("INCORRECT");
VanGui::DebugTextEncoding("こんにちは");
```
![UTF-8 Encoding viewer](https://github.com/ocornut/vangui/assets/8225057/61c1696a-9a94-46c5-9627-cf91211111f0)
...
- Since May 2023 we have changed the Visual Studio projects of all our examples to use `/utf-8` ([see commit](https://github.com/ocornut/vangui/commit/513af1efc9080857bbd10000d98f98f2a0c96803)).

Or, since C++11, you can use the `u8"my text"` syntax to encode literal strings as UTF-8. e.g.:
```cpp
VanGui::Text(u8"hello");
VanGui::Text(u8"こんにちは");   // this will always be encoded as UTF-8
VanGui::Text("こんにちは");     // the encoding of this is depending on compiler settings/flags and may be incorrect.
```
...
```cpp
VanGui::Text((const char*)u8"こんにちは");
```
...
You can use the `UTF-8 Encoding viewer` in `Metrics/Debugger` to verify the content of your UTF-8 strings. From C/C++ code, you can call `VanGui::DebugTextEncoding("my string");` function to verify that your UTF-8 encoding is correct.
