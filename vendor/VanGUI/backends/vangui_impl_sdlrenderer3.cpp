// dear vangui: Renderer Backend for SDL_Renderer for SDL3
// (Requires: SDL 3.1.8+)
...
//  2026-03-12: Fixed invalid assert in VanGui_ImplSDLRenderer3_UpdateTexture() if VanTextureID_Invalid is defined to be != 0, which became the default since 2026-03-12. (#9295)
//  2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-06-11: Added support for VanGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed VanGui_ImplSDLRenderer3_CreateFontsTexture() and VanGui_ImplSDLRenderer3_DestroyFontsTexture().
//  2025-01-18: Use endian-dependent RGBA32 texture format, to match SDL_Color.
//  2024-10-09: Expose selected render state in VanGui_ImplSDLRenderer3_RenderState, which you can access in 'void* platform_io.Renderer_RenderState' during draw callbacks.
...
#include "vangui_impl_sdlrenderer3.h"
...
// SDL_Renderer data
struct VanGui_ImplSDLRenderer3_Data
{
    SDL_Renderer*           Renderer;       // Main viewport's renderer
    VanVector<SDL_FColor>    ColorBuffer;

    // Render State
    SDL_ScaleMode           CurrentScaleMode;

    VanGui_ImplSDLRenderer3_Data()   { memset(static_cast<void*>(this), 0, sizeof(*this)); }
};
...
static VanGui_ImplSDLRenderer3_Data* VanGui_ImplSDLRenderer3_GetBackendData()
{
    return VanGui::GetCurrentContext() ? static_cast<VanGui_ImplSDLRenderer3_Data*>(VanGui::GetIO().BackendRendererUserData) : nullptr;
}

// Functions
static void VanGui_ImplSDLRenderer3_SetupRenderState(SDL_Renderer* renderer)
{
...
void VanGui_ImplSDLRenderer3_NewFrame()
{
    VanGui_ImplSDLRenderer3_Data* bd = VanGui_ImplSDLRenderer3_GetBackendData();
    VAN_ASSERT(bd != nullptr && "Context or backend not initialized! Did you call VanGui_ImplSDLRenderer3_Init()?");
    VAN_UNUSED(bd);
}

// https://github.com/libsdl-org/SDL/issues/9009
static int SDL_RenderGeometryRaw8BitColor(SDL_Renderer* renderer, VanVector<SDL_FColor>& colors_out, SDL_Texture* texture, const float* xy, int xy_stride, const SDL_Color* color, int color_stride, const float* uv, int uv_stride, int num_vertices, const void* indices, int num_indices, int size_indices)
{
    const Uint8* color2 = reinterpret_cast<const Uint8*>(color);
    ...
    for (int i = 0; i < num_vertices; i++)
    {
        ...
        color2 += color_stride;
        color = reinterpret_cast<const SDL_Color*>(color2);
    }
...
// Draw callbacks
static void VanGui_ImplSDLRenderer3_DrawCallback_ResetRenderState(const VanDrawList*, const VanDrawCmd*)   {} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to implement this way.
static void VanGui_ImplSDLRenderer3_DrawCallback_SetSamplerLinear(const VanDrawList*, const VanDrawCmd*)   { VanGui_ImplSDLRenderer3_Data* bd = VanGui_ImplSDLRenderer3_GetBackendData(); bd->CurrentScaleMode = SDL_SCALEMODE_LINEAR; }
static void VanGui_ImplSDLRenderer3_DrawCallback_SetSamplerNearest(const VanDrawList*, const VanDrawCmd*)  { VanGui_ImplSDLRenderer3_Data* bd = VanGui_ImplSDLRenderer3_GetBackendData(); bd->CurrentScaleMode = SDL_SCALEMODE_NEAREST; }

void VanGui_ImplSDLRenderer3_RenderDrawData(VanDrawData* draw_data, SDL_Renderer* renderer)
{
    VanGui_ImplSDLRenderer3_Data* bd = VanGui_ImplSDLRenderer3_GetBackendData();
    ...
    if (draw_data->Textures != nullptr)
        for (VanTextureData* tex : *draw_data->Textures)
            if (tex->Status != VanTextureStatus_OK)
                VanGui_ImplSDLRenderer3_UpdateTexture(tex);
    ...
    // Setup desired state
    VanGui_ImplSDLRenderer3_SetupRenderState(renderer);

    // Setup render state structure (for callbacks and custom texture bindings)
    VanGuiPlatformIO& platform_io = VanGui::GetPlatformIO();
    VanGui_ImplSDLRenderer3_RenderState render_state;
    ...
    for (const VanDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback)
            {
                ...
                if (pcmd->UserCallback == VanGui_ImplSDLRenderer3_DrawCallback_ResetRenderState)
                    VanGui_ImplSDLRenderer3_SetupRenderState(renderer);
                else
...
void VanGui_ImplSDLRenderer3_UpdateTexture(VanTextureData* tex)
{
    VanGui_ImplSDLRenderer3_Data* bd = VanGui_ImplSDLRenderer3_GetBackendData();
...
void VanGui_ImplSDLRenderer3_CreateDeviceObjects()
{
}

void VanGui_ImplSDLRenderer3_DestroyDeviceObjects()
{
...
bool VanGui_ImplSDLRenderer3_Init(SDL_Renderer* renderer)
{
    ...
    VanGui_ImplSDLRenderer3_Data* bd = VAN_NEW(VanGui_ImplSDLRenderer3_Data)();
    io.BackendRendererUserData = static_cast<void*>(bd);
    io.BackendRendererName = "vangui_impl_sdlrenderer3";
    ...
    VanGuiPlatformIO& platform_io = VanGui::GetPlatformIO();
    platform_io.DrawCallback_ResetRenderState = VanGui_ImplSDLRenderer3_DrawCallback_ResetRenderState;
    platform_io.DrawCallback_SetSamplerLinear = VanGui_ImplSDLRenderer3_DrawCallback_SetSamplerLinear;
    platform_io.DrawCallback_SetSamplerNearest = VanGui_ImplSDLRenderer3_DrawCallback_SetSamplerNearest;
...
void VanGui_ImplSDLRenderer3_Shutdown()
{
    VanGui_ImplSDLRenderer3_Data* bd = VanGui_ImplSDLRenderer3_GetBackendData();
    ...
    VanGui_ImplSDLRenderer3_DestroyDeviceObjects();
