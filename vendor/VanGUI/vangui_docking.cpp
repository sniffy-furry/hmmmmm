...
// (docking and viewport code)

/*
Index of this file:

// [SECTION] Docking: Internal Types
// [SECTION] Docking: Forward Declarations
// [SECTION] Docking: Context
// [SECTION] Docking: Settings
// [SECTION] Docking: Node creation, splitting, merging
// [SECTION] Docking: Window docking/undocking
// [SECTION] Docking: Rendering and hit-testing
...
// Forward declaration: DockNodeUpdate is defined later in this file
static void DockNodeUpdate(VanGuiDockNode* node);

static VanGuiDockNode* DockContextAddNode(VanGuiContext* ctx, VanGuiID id)
{
    VanGuiContext& g = *ctx;
    // Generate a unique ID. Use the context's own counter so it resets with the context.
    if (id == 0)
    {
        do { id = ++ctx->DockContext.NodeIdNext; }
        while (id == 0 || DockContextFindNodeByID(ctx, id) != nullptr);
    }
...
VanGuiID DockSpaceOverViewport(VanGuiID dockspace_id, const VanGuiViewport* viewport, VanGuiDockNodeFlags flags, const VanGuiWindowClass* window_class)
{
    ...
    (void)(Begin(label, nullptr, host_window_flags));
    PopStyleVar(3);
                                          ...
                                          void* entry, const char* line)
{
    ...
    if (sscanf_s(line, "Parent=0x%08X", &u) == 1)
    {
    ...
    else if (sscanf_s(line, "SizeRef=%i,%i", &x, &y) == 2)
    {
        ns->SizeRef = VanVec2ih(static_cast<short>(x), static_cast<short>(y));
    }
    else if (sscanf_s(line, "Split=%c", &axis_ch, 1u) == 1)
    {
    ...
    else if (sscanf_s(line, "Selected=0x%08X", &u) == 1)
    {
        ns->SelectedTabId = static_cast<VanGuiID>(u);
    }
    else if (sscanf_s(line, "Flags=0x%08X", &u) == 1)
    {
