...
#### Building for desktop (WebGPU-native) with Google Dawn:
 ...
 2. `cmake -B build -DVANGUI_DAWN_DIR=dawn`
 3. `cmake --build build`
...
#### Building for desktop (WebGPU-Native) with WGPU:
 ...
 3. `cmake -B build -DVANGUI_WGPU_DIR=your_preferred_folder`  ("full path" or "relative" starting from current directory)
 4. `cmake --build build`
The resulting binary will be found at one of the following locations:
  * build/Debug/example_sdl2_wgpu[.exe]
  * build/example_sdl2_wgpu[.exe]
...
#### Building for Emscripten:
 ...
 3. `emcmake cmake -G Ninja -B build`
    - (optional) `-DVANGUI_EMSCRIPTEN_WEBGPU_FLAG="--use-port=path/to/emdawnwebgpu_package/emdawnwebgpu.port.py"`, see below
 4. `cmake --build build`
...
3. `emcmake cmake -DVANGUI_EMSCRIPTEN_WEBGPU_FLAG="--use-port=path/to/emdawnwebgpu_package/emdawnwebgpu.port.py" -G Ninja -B build`
...
- `cmake -G Ninja -DVANGUI_DAWN_DIR=path_to_sdk_dir -B where_to_build_dir`
   - Using `VANGUI_DAWN_DIR` set `VANGUI_IMPL_WEBGPU_BACKEND_DAWN` compiler define 

#### Generate WGPU Native:

- `cmake -G Ninja -DVANGUI_WGPU_DIR=path_to_sdk_dir -B where_to_build_dir`

   - Using `VANGUI_WGPU_DIR` set `VANGUI_IMPL_WEBGPU_BACKEND_WGPU` compiler define 
...
CMake checks the EMSCRIPEN version then:
   - if EMS >= 4.0.10 uses `--use-port=emdawnwebgpu` flag to build  
      - it set `VANGUI_IMPL_WEBGPU_BACKEND_DAWN` compiler define 
...
- `emcmake cmake -G Ninja -DVANGUI_EMSCRIPTEN_WEBGPU_FLAG="--use-port=path_to_emdawnwebgpu_pkg" -B where_to_build_dir`
   - it set `VANGUI_IMPL_WEBGPU_BACKEND_DAWN` compiler define 
...
- using **make** instead **ninja**:
   - `cmake -G "Unix Makefiles" -DVANGUI_DAWN_DIR=path_to_sdk_dir -B where_to_build_dir`
...
Example:
- building **Release**:
   - `cmake -G ninja -DVANGUI_WGPU_DIR=path_to_sdk_dir -DCMAKE_BUILD_TYPE=Release -B where_to_build_dir ` 
...
- full cmake command using **vcpkg** package manager:
   - `cmake -G Ninja -DVANGUI_DAWN_DIR=path_to_sdk_dir -DCMAKE_TOOLCHAIN_FILE=<vcpkg_root_dir>/scripts/buildsystems/vcpkg.cmake -B where_to_build_dir`
