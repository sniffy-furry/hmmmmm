// VanGUI: standalone example application for GLFW + WebGPU
...
int main(int, char**)
{
    ...
    GLFWwindow* window = glfwCreateWindow(wgpu_surface_width, wgpu_surface_height, "VanGUI GLFW+WebGPU example", nullptr, nullptr);
    if (window == nullptr)
        return 1;
    ...
    VanGui_ImplGlfw_InitForOther(window, true);
#ifdef __EMSCRIPTEN__
    VanGui_ImplGlfw_InstallEmscriptenCallbacks(window, "#canvas");
#endif
    VanGui_ImplWGPU_InitInfo init_info;
    init_info.Device = wgpu_device;
    init_info.NumFramesInFlight = 3;
    init_info.RenderTargetFormat = wgpu_surface_configuration.format;
    init_info.DepthStencilFormat = WGPUTextureFormat_Undefined;
    VanGui_ImplWGPU_Init(&init_info);
...
#ifndef VANGUI_DISABLE_FILE_FUNCTIONS
    ...
    //VanFont* font = io.Fonts->AddFontFromFileTTF("fonts/ArialUni.ttf");
    ...
    // Main loop
#ifdef __EMSCRIPTEN__
    // For an Emscripten build we are disabling file-system access, so let's not attempt to do a fopen() of the vangui.ini file.
...
#endif
    {
        ...
        if (VanGui_ImplWGPU_IsSurfaceStatusError(surface_texture.status))
        {
            fprintf(stderr, "Unrecoverable Surface Texture status=%#.8x\n", surface_texture.status);
            abort();
        }
        if (VanGui_ImplWGPU_IsSurfaceStatusSubOptimal(surface_texture.status))
        {
        ...
        VanGui_ImplWGPU_RenderDrawData(VanGui::GetDrawData(), pass);
...
static WGPUAdapter RequestAdapter(wgpu::Instance& instance)
{
    ...
    VAN_ASSERT(acquired_adapter != nullptr && waitStatusAdapter == wgpu::WaitStatus::Success && "Error on Adapter request");
...
static WGPUDevice RequestDevice(wgpu::Instance& instance, wgpu::Adapter& adapter)
{
    ...
    VAN_ASSERT(acquired_device != nullptr && waitStatusDevice == wgpu::WaitStatus::Success && "Error on Device request");
    return acquired_device.MoveToCHandle();
}
#elif defined(VANGUI_IMPL_WEBGPU_BACKEND_WGPU) || defined(VANGUI_IMPL_WEBGPU_BACKEND_WGVK)
static void handle_request_adapter(WGPURequestAdapterStatus status, WGPUAdapter adapter, WGPUStringView message, void* userdata1, void* userdata2)
{
    VAN_UNUSED(userdata2);
...
static WGPUAdapter RequestAdapter(WGPUInstance& instance)
{
    ...
    VAN_ASSERT(local_adapter && "Error on Adapter request");
...
static WGPUDevice RequestDevice(WGPUInstance& instance, WGPUAdapter& adapter)
{
    ...
    WGPUFutureWaitInfo waitInfo = { future, false };
    wgpuInstanceWaitAny(instance, 1, &waitInfo, ~0ull);
    VAN_ASSERT(local_device && "Error on Device request");
    return local_device;
}
#endif // VANGUI_IMPL_WEBGPU_BACKEND_WGPU
...
bool InitWGPU(GLFWwindow* window)
{
    ...
    VanGui_ImplWGPU_DebugPrintAdapterInfo(adapter.Get());
...
#if defined(VANGUI_IMPL_WEBGPU_BACKEND_WGPU)
    wgpuSetLogCallback(
        [](WGPULogLevel level, WGPUStringView msg, void* userdata) { fprintf(stderr, "%s: %.*s\n", VanGui_ImplWGPU_GetLogLevelName(level), (int)msg.length, msg.data); }, nullptr
    );
    wgpuSetLogLevel(WGPULogLevel_Warn);
#endif

    WGPUAdapter adapter = RequestAdapter(wgpu_instance);
    VanGui_ImplWGPU_DebugPrintAdapterInfo(adapter);
...
WGPUSurface CreateWGPUSurface(const WGPUInstance& instance, GLFWwindow* window)
{
    VanGui_ImplWGPU_CreateSurfaceInfo create_info = {};
    create_info.Instance = instance;
#if defined(GLFW_EXPOSE_NATIVE_COCOA)
    {
        create_info.System = "cocoa";
        create_info.RawWindow = (void*)glfwGetCocoaWindow(window);
        return VanGui_ImplWGPU_CreateWGPUSurfaceHelper(&create_info);
    }
