...
// (widgets code)
// True C++23 module interface partition — vangui:widgets
// Merged from vangui-widgets.ixx + vangui_widgets.cpp

// Regular C++ TU — symbols get standard external linkage (no ::<!vangui> mangling).
// The thin interface vangui-widgets.ixx exports these names; the linker resolves them here.
...
// Remove export block — it is now in vangui-widgets.ixx (thin interface).
// The following lines are the START of the discarded export block; we skip to
// the closing brace below.
namespace VanGui_DISCARD_EXPORT_BLOCK {
...
} // namespace VanGui_DISCARD_EXPORT_BLOCK — actual exports live in vangui-widgets.ixx

//=============================================================================
// IMPLEMENTATION — function bodies (formerly vangui_widgets.cpp)
#ifndef VANGUI_DISABLE
//=============================================================================
...
bool VanGui::ColorPicker4(const char* label, float col[4], VanGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & VanGuiColorEditFlags_PickerHueWheel)
    {
        ...
        InvisibleButton("hsv", VanVec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size));
    ...
    {
        // SV rectangle logic
        InvisibleButton("sv", VanVec2(sv_picker_size, sv_picker_size));
        if (IsItemActive() && !is_readonly)
        {
        ...
        InvisibleButton("hue", VanVec2(bars_width, sv_picker_size));
        if (IsItemActive() && !is_readonly)
        {
    ...
    if (alpha_bar)
    {
        ...
        InvisibleButton("alpha", VanVec2(bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    if (!(flags & VanGuiColorEditFlags_NoSidePreview))
    {
        ...
        ColorButton("##current", col_v4, (flags & sub_flags_to_forward), VanVec2(square_sz * 3, square_sz * 2));
        if (ref_col != nullptr)
        {
            Text("Original");
...
bool VanGui::ColorButton(const char* desc_id, const VanVec4& col, VanGuiColorEditFlags flags, const VanVec2& size_arg)
{
    ...
    if (g.ActiveId == id && !(flags & VanGuiColorEditFlags_NoDragDrop) && BeginDragDropSource())
    {
        if (flags & VanGuiColorEditFlags_NoAlpha)
            SetDragDropPayload(VANGUI_PAYLOAD_TYPE_COLOR_3F, &col_rgb, sizeof(float) * 3, VanGuiCond_Once);
        else
            SetDragDropPayload(VANGUI_PAYLOAD_TYPE_COLOR_4F, &col_rgb, sizeof(float) * 4, VanGuiCond_Once);
...
void VanGui::ColorTooltip(const char* text, const float* col, VanGuiColorEditFlags flags)
{
    ...
    ColorButton("##preview", cf, (flags & flags_to_forward) | VanGuiColorEditFlags_NoTooltip, sz);
    SameLine();
...
void VanGui::ColorPickerOptionsPopup(const float* ref_col, VanGuiColorEditFlags flags)
{
    ...
    if (allow_opt_alpha_bar)
    {
        ...
        CheckboxFlags("Alpha Bar", &g.ColorEditOptions, VanGuiColorEditFlags_AlphaBar);
    }
    PopItemFlag();
    EndPopup();
}
...
bool VanGui::BeginListBox(const char* label, const VanVec2& size_arg)
{
    ...
    BeginChild(id, frame_bb.GetSize(), VanGuiChildFlags_FrameStyle);
    return true;
}
